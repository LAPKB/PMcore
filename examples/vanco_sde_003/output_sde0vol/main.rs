...
pub(crate) fn main() {
    let sde = equation::SDE::new(
        |x, p, _t, dx, rateiv, cov| {
            fetch_params!(p, _ke0, v0, kcp, well);
            fetch_cov!(cov,_t,scr,wt);
            dx[0] = 0.0; // ke0 - x[0]; // mean reverting sde
            dx[1] = v0 - x[1];
            let ke = x[0];
            let _vol = x[1];
            let kpc = well * kcp;
        ...
        |p, d| {
            fetch_params!(p, _ke0, _vol, _well, _ske, svol);
            d[0] = 0.0; // ske;
            d[1] = svol;
    ...
    settings.set_output_path("examples/vanco_sde_003/output_sde0vol"); // *** SET OUTPUT DIRECTORY HERE ***
    //
