...
pub(crate) fn main() -> Result<()> {
    ...
    settings.set_prior(Prior::File(
        "examples/vanco_sde_004/output_ode_sobol730/theta_s0.csv".to_string(),
    ));
    settings.set_output_path("examples/vanco_sde_004/output_sde0");
    settings.set_write_logs(true);
    settings.set_log_level(LogLevel::DEBUG);
    settings.initialize_logs()?;

    let data = data::read_pmetrics("examples/vanco_sde_004/vclean.csv")?;
