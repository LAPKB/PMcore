...
fn settings_exp2() -> Settings {
    ...
    let params = Parameters::builder()
        ...
        .add("v0", 0.8, 1.2, false)
         .add("s1", 0.0001, 0.075, false)        // Diffusion parameter for ke
        .build()
        .unwrap();
...
fn settings_exp5() -> Settings {
    ...
    let params = Parameters::builder()
        ...
        .add("s1", 2.5e-2, 0.3, false) // Diffusion parameter for ke
        .add("s2", 0.1, 0.4, false) // Diffusion parameter for volume
        .build()
        .unwrap();
...
*/
    ...
    write_samples_to_file(
        ...
        k0_pop.clone(),
        Some(v_pop.clone()),
        0.05, // mu <- c(0.5 +/- 0.05) or mu <- c(1.5 +/- 0.15)
        0.20, // sigma_vol is hard coded to 0.2 => 20%CV, vol = 1+/-s
        model_ke_v_s1_s2(),
    );
}
...
fn fit_experiment(experiment: usize) {
    ...
    if experiment == 0 {
        let data = data::read_pmetrics("examples/sde_paper/data/experiment2.csv").unwrap();
        let eqn = model_ke_ode();
        let mut settings = settings_exp1();
        settings.set_output_path("examples/sde_paper/output/experiment0");
        settings.set_error_poly((4.0, 0.32, 0.0, 0.0));
