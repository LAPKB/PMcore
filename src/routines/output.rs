...
impl NPResult {
    ...
    pub fn summary(&self, equation: &Equation) {
        ...
        let mut mod_table = Table::new(&[ModelSummaryRow {
            objf: format!("{:.2}", self.objf),
            aic: format!("{:.2}", aic),
            bic: format!("{:.2}", bic),
            nspp: format!("{}", self.theta.nrows()),
        }]);

        tracing::info!(
            "{}\n{}",
            "Model summary",
            mod_table.with(Style::modern()).to_string()
        );
        ...
        let pop_table = create_pred_err_table(
            &pop_mean_mpe_mae,
            &pop_median_mpe_mae,
            "Population Predictions",
        );
        ...
        let post_table = create_pred_err_table(
            &post_mean_mpe_mae,
            &post_median_mpe_mae,
            "Posterior Predictions",
        );
...
fn create_pred_err_table(
    ...
    for (outeq, (mpe, mae)) in mean_data {
        rows.push(PredictionSummaryRow {
            outeq: *outeq,
            icen: "mean",
            mae: format!("{:.2}", mae),
            mpe: format!("{:.2}", mpe),
