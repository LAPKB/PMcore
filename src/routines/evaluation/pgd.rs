...
pub fn pgd_weights(psi: &Psi) -> Result<(Weights, f64)> {
    ...
    let mut s = vec![0.0; n];
    let mut g = vec![0.0; m];
    // initial step, will be adapted to avoid huge updates on extreme matrices
    let mut step = 0.5; // start with a conservative step
    for _iter in 0..max_iter {
        ...
        // Stabilize step by estimating a local Lipschitz-like constant via max gradient magnitude
        let max_g_minus_one = g.iter().map(|&gj| (gj - 1.0).abs()).fold(0.0_f64, f64::max);
        if max_g_minus_one > 0.0 {
            // limit maximum per-coordinate update to avoid creating extremely large intermediate values
            let max_update = 1e1_f64;
        ...
        // build update vector robustly; clamp large values to a reasonable range before projection
        // Backtracking line-search: try to accept step only if objective improves (or stays approximately the same)
        ...
        for _trial in 0..40 {
            ...
            let clamp_bound = 1e200;
            for (&xj, &gj) in x.iter().zip(g.iter()) {
                let mut val = xj + local_step * (gj - 1.0);
                if !val.is_finite() {
                    val = eps;
                }
                if val > clamp_bound {
            ...
            let obj_candidate = s_new.iter().map(|&si| si.ln()).sum::<f64>();

            // Accept if objective didn't decrease (allow tiny numerical tolerance)
            if obj_candidate >= obj_current - 1e-12 {
                accepted = true;
                // use s_new as current s for next iteration
                s = s_new;
                break;
            }
            // otherwise, reduce step and try again
            local_step *= 0.5;
            if local_step < 1e-20 {
                break;
            }
        }
        // If not accepted, use the best candidate we have (x_new) but ensure s is recomputed
        if !accepted {
            for i in 0..n {
                let mut si = 0.0;
                for j in 0..m {
        ...
        obj_prev = obj;
        x = x_new;
        // adapt step mildly: if last local_step was accepted and not tiny, increase a bit
        if accepted {
