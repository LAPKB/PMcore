    ...
    S: Predict + std::marker::Sync + std::marker::Send + 'static,
{
    ...
    if std::path::Path::new("stop").exists() {
        match std::fs::remove_file("stop") {
            ...
            Err(err) => log::error!("Unable to remove previous stop file: {}", err),
        }
    }
    ...
    if let Some(output) = &settings.parsed.config.pmetrics_outputs {
        if *output {
            ...
            // theta.csv
            match write_theta(&theta, &w, &settings) {
                Ok(_) => log::info!("Population parameters written to theta.csv"),
                Err(e) => log::error!("Error writing theta.csv : {}", e),
            }

            // posterior.csv
            match write_posterior(&psi, &w, &settings, &theta, &scenarios) {
                Ok(_) => log::info!("Posterior parameters written to posterior.csv"),
                Err(e) => log::error!("Error writing posterior.csv : {}", e),
            }

            // pred.csv
            match write_pred(&theta, &w, &psi, sim_eng, &scenarios, cache) {
                Ok(_) => log::info!("Predictions written to pred.csv"),
                Err(e) => log::error!("Error writing pred.csv : {}", e),
            }

            //obs.csv
            match write_obs(&scenarios) {
                Ok(_) => log::info!("Observations written to obs.csv"),
                Err(e) => log::error!("Error writing obs.csv : {}", e),
            }
        }
    }
}
