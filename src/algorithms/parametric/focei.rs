...
use crate::estimation::parametric::{
	assemble_focei_result, batch_log_likelihood_from_eta, covariate_state,
	covariance_from_individual_etas, covariance_from_subject_means,
	ensure_positive_definite_covariance, estimate_beta, occasion_covariate_maps,
	recenter_individual_estimates, subject_covariate_maps, subject_mean_phi,
	subject_objective_from_eta,
	FoceiResultInput, Individual, IndividualEstimates, LikelihoodEstimates,
	ParameterTransform, ParametricIterationLog, ParametricWorkspace, Population,
};
...
impl<E: Equation + Send + 'static> FoceiAlgorithm<E> {
	pub(crate) fn create(input: ParametricAlgorithmInput<E>) -> Result<Box<Self>> {
		...
		let (mut subject_covariate_model, occasion_covariate_model, subject_covariates, occasion_covariates) = match covariates {
		...
		if let Some(model) = subject_covariate_model.as_mut() {
			let initialize_intercepts = (0..model.beta().nrows())
				.all(|index| model.beta()[index].abs() < 1e-12);
	...
	fn find_map_estimate(
		...
		let eta = Col::from_fn(n_params, |param_index| eta_matrix[[subject_index, param_index]]);
	...
	fn update_population_parameters(&mut self) -> Result<()> {
		...
		if let Some(model) = self.subject_covariate_model.clone() {
			...
			self.population.update_omega(ensure_positive_definite_covariance(&omega))?;
		} else {
...
impl<E: Equation + Send + 'static> ParametricAlgorithm<E> for FoceiAlgorithm<E> {
	...
	fn e_step(&mut self) -> Result<()> {
		...
		for (subject_index, _subject) in subjects.iter().enumerate() {
			let individual = self.find_map_estimate(
				subject_index,
				&mut eta_matrix,
				&omega_inv,
				&subject_means,
